...
/**
 * The Power Lora Stacker is a super-simply Lora Stacker node that can load multiple Loras at once
...
class RgthreePowerLoraStacker extends RgthreeBaseServerNode {
  ...
  override configure(info: ISerialisedNode): void {
    ...
    for (const widgetValue of info.widgets_values || []) {
      if ((widgetValue as PowerLoraStackerWidgetValue)?.lora !== undefined) {
        const widget = this.addNewLoraWidget();
        widget.value = {...(widgetValue as PowerLoraStackerWidgetValue)};
      }
    }
